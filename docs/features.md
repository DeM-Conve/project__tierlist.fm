...
- `/add` (sidebar "Add songs", `a` from anywhere, Home's "Add songs" button, command palette): **one input** - paste a YouTube / YouTube Music link (or several: one per line / space-separated; watch, youtu.be, music, shorts links) and they're added; or type a song name and press `Enter` to **search** (Music category only, so a name finds the song, not clips; each query cached for the session - YouTube charges 100 quota units per search). `↑`/`↓` pick a result, `Enter` adds it; results already in a tier say where.
- **`Ctrl+V` with a link on any page** (outside a text box) adds it and opens Add songs.
- Each picked song becomes a card: cover (click / `Enter` to listen in the mini player), song + artist, the **board guessed from the artist** (most songs by them; else where the last one went; `b` / the picker changes it) and big tier buttons: `1`-`5` puts it straight into that tier on YouTube (no pending step), `t` "Later" = the board's TODO list. `s` skips, `x` removes it from the waiting list, `/` or `a` back to the input. Putting the playing song in a tier plays the next waiting one.
- A song that's already in a tier shows "Already in <board> · <tier>" with Open it / Dismiss instead of tier buttons.
...
- Expanded view: a **queue panel** beside the video, as tall as the player - one continuous numbered list, YouTube-Music style: songs already played sit above (dimmed, "N earlier" beyond the last 30), the playing song is highlighted in place (equalizer mark), and what's next follows below; the list auto-scrolls so the playing song stays near the top as the queue advances (click any entry to jump to it, hover × to remove it from the queue; shows position and a Shuffle badge) - plus on-screen transport controls (prev, back 10s, play/pause, forward 10s, next, volume) and a backdrop glow in the playing song's tier color.
...
- Navigation (Vimium-style, anywhere outside a text box): `f` **link hints** - every visible clickable (tiles, buttons, tabs, inputs, sidebar links) gets a yellow letter label, type it to click/focus, `Backspace` edits, `Esc`/scroll/click cancels; `o` command palette; `a` Add songs, `g h` Home, `g s` Settings, `g p` playing song's board, `g b` this board, `g t` its TODO list, `g d` duel; `[` / `]` previous/next board; `H` / `L` back/forward; `g g` / `G` top/bottom; `y y` copy the playing song's YouTube link. The key after a `g`/`y` prefix is never also taken by a page shortcut (e.g. `g h` doesn't also skip to the previous track).
- Add songs: `1`-`5` put in T1…TZ, `t` save to TODO, `Enter` listen, `s` skip, `x` remove, `b` pick board, `/` or `a` the input, `u` / `Ctrl+Z` undo.
