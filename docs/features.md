...
## Settings
- **Appearance** tab (default): pick a **theme** (Graphite, Midnight, Pure black, Warm charcoal - dark; Paper, Sand - light), an **accent** (Violet, Fuchsia, Cyan, Mono - works with any theme) and a **tier palette** (Vivid, TierMaker classic, Heat). Each option shows a live mini preview, changes apply instantly across the whole app (including the share image) and are remembered in this browser. All colors come from one file, `frontend/src/themes.js`.
